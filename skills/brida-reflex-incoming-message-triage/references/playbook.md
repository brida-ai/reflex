# Incoming message triage playbook

Canonical use-case ID: `incoming-message-triage`
Version: `1`
...
classify one already-admitted non-sensitive incoming message by the kind of attention it appears to need after exact transport, security, sender-policy and channel rules have already run.

## Primary semantic question

Classify the supplied already-admitted incoming message by the kind of attention it appears to need, based only on the supplied bounded message context after deterministic security and routing checks have run.
