---
name: brida-reflex-code-change-classification
description: "Implement and verify the Brida Reflex Code change classification use case: classify the primary semantic type of one bounded software change after deterministic Git, path and static-analysis checks. Use when the user names code-change-classification, asks for this exact bounded decision, or wants to integrate/test the matching Reflex without a discovery interview. Triggers include: commit classification, diff classification, change type."
---
