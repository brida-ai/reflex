---
name: brida-reflex-outbound-message-risk
description: "Implement and verify the Brida Reflex Outbound message risk use case: classify whether one bounded outgoing draft is suitable to send as written, needs human review, or contains a material semantic risk that should be blocked by host policy. Use when the user names outbound-message-risk, asks for this exact bounded decision, or wants to integrate/test the matching Reflex without a discovery interview. Triggers include: send guard, outbound message review, message risk."
---
